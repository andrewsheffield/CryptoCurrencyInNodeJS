...
CoinClient.prototype.on('proof', function(self, proof) {
  var ledger = proof.ledger;
  var proof = proof.proof;
  var hashVal = self.hash(JSON.stringify(ledger) + proof);

  var expectedHash = "";
  for (j=ZEROES_REQ; j > 0; j--) {
    expectedHash += "0";
  }

  if (self.chainLength(ledger) > self.chainLength(this.ledger)
    && hashVal.substr(0, ZEROES_REQ) == expectedHash) {
    console.log("New ledger accepted for proof " + proof);
    this.ledger = ledger;
    this.broadcast({type: 'shareledger', 'ledger':self.ledger});
  } else {
    this.broadcast({type: 'reject', msg: 'bad proof'});
  }
});
...
CoinClient.prototype.validateTransfer = function(trans) {
  ...
  verifier.update(msg);
  var ver = verifier.verify(this.pubKey, trans.sig, 'hex');
...
CoinClient.prototype.mineProof = function(newLedger, start) {
  ...
  while (hashVal.substr(0, ZEROES_REQ) !== expectedHash) {
    proof++;
    hashVal = this.hash(ledge+proof);
  }
